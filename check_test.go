// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

package checkbook_test

import (
	"context"
	"errors"
	"os"
	"testing"
	"time"

	"github.com/Munchpass/checkbook"
	"github.com/Munchpass/checkbook/internal/testutil"
	"github.com/Munchpass/checkbook/option"
)

func TestCheckGet(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.Get(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckListWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.List(context.TODO(), checkbook.CheckListParams{
		Direction: checkbook.CheckListParamsDirectionOutgoing,
		EndDate:   checkbook.Time(time.Now()),
		Page:      checkbook.Int(1),
		PerPage:   50,
		Q:         checkbook.String("payment"),
		Sort:      checkbook.CheckListParamsSortPlusDate,
		StartDate: checkbook.Time(time.Now()),
		Status:    checkbook.CheckListParamsStatusPaid,
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckNewDigitalWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.NewDigital(context.TODO(), checkbook.CheckNewDigitalParams{
		CreateDigitalCheck: checkbook.CreateDigitalCheckParam{
			Amount: 150,
			Name:   "Dwight Schrute",
			Recipient: checkbook.CreateDigitalCheckRecipientUnionParam{
				OfString: checkbook.String("dwight@example.com"),
			},
			Account: checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Attachment: checkbook.CreateDigitalCheckAttachmentUnionParam{
				OfAttachment: &checkbook.AttachmentParam{
					Content:  "",
					Filename: "attachment.pdf",
				},
			},
			Comment:        checkbook.String("comment"),
			DepositOptions: []string{"BANK", "RTP"},
			Description:    checkbook.String("Example memo"),
			Number: checkbook.CreateDigitalCheckNumberUnionParam{
				OfString: checkbook.String("5001"),
			},
			Pin: checkbook.PinParam{
				Description: "Please enter your payments pin #",
				Value:       "1234",
			},
			RemittanceAdvice: []checkbook.RemittanceAdviceParam{{
				ID:          "id",
				Date:        "date",
				Amount:      checkbook.Float(0),
				Description: checkbook.String("description"),
			}},
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckNewMultiWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.NewMulti(context.TODO(), checkbook.CheckNewMultiParams{
		CreateMultiCheck: checkbook.CreateMultiCheckParam{
			Amount: 5.23,
			Recipients: []checkbook.MultiRecipientParam{{
				Name:        "Widgets Inc.",
				Recipient:   "testing@example.com",
				EndorseOnly: checkbook.Bool(true),
				Pin: checkbook.PinParam{
					Description: "Please enter your payments pin #",
					Value:       "1234",
				},
			}, {
				Name:        "Widgets Inc.",
				Recipient:   "testing@example.com",
				EndorseOnly: checkbook.Bool(true),
				Pin: checkbook.PinParam{
					Description: "Please enter your payments pin #",
					Value:       "1234",
				},
			}},
			Account: checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Attachment: checkbook.CreateMultiCheckAttachmentUnionParam{
				OfAttachment: &checkbook.AttachmentParam{
					Content:  "",
					Filename: "attachment.pdf",
				},
			},
			Comment:        checkbook.String("comment"),
			DepositOptions: []string{"PRINT"},
			Description:    checkbook.String("Example memo"),
			Number: checkbook.CreateMultiCheckNumberUnionParam{
				OfString: checkbook.String("5001"),
			},
			RemittanceAdvice: []checkbook.RemittanceAdviceParam{{
				ID:          "id",
				Date:        "date",
				Amount:      checkbook.Float(0),
				Description: checkbook.String("description"),
			}},
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckNewPhysicalWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.NewPhysical(context.TODO(), checkbook.CheckNewPhysicalParams{
		CreatePhysicalCheck: checkbook.CreatePhysicalCheckParam{
			Amount: 5.23,
			Name:   "Widgets Inc.",
			Recipient: checkbook.PhysicalCheckRecipientParam{
				City:    "Anytown",
				Line1:   "1234 N. 1st Street",
				State:   "CA",
				Zip:     "55555",
				Country: checkbook.String("country"),
				Line2:   checkbook.String("Suite 56"),
				Name:    checkbook.String("Widgets Inc."),
			},
			Account: checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Attachment: checkbook.CreatePhysicalCheckAttachmentUnionParam{
				OfAttachment: &checkbook.AttachmentParam{
					Content:  "",
					Filename: "attachment.pdf",
				},
			},
			Comment:     checkbook.String("comment"),
			Description: checkbook.String("Example memo"),
			MailType:    checkbook.CreatePhysicalCheckMailTypeUspsFirstClass,
			Number: checkbook.CreatePhysicalCheckNumberUnionParam{
				OfString: checkbook.String("5001"),
			},
			RemittanceAdvice: checkbook.CreatePhysicalCheckRemittanceAdviceUnionParam{
				OfString: checkbook.String("string"),
			},
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckEndorseWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	err := client.Check.Endorse(
		context.TODO(),
		"check_id",
		checkbook.CheckEndorseParams{
			Name:      "Johnny Appleseed",
			Signature: checkbook.String("signature"),
		},
	)
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckGetAttachment(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.GetAttachment(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckGetFailDetails(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.GetFailDetails(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckGetTrackingDetails(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.GetTrackingDetails(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckGetVerificationCode(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.GetVerificationCode(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckNotify(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	err := client.Check.Notify(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckPreviewWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.Preview(context.TODO(), checkbook.CheckPreviewParams{
		Amount:      0.01,
		Name:        "Widgets Inc.",
		Account:     checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
		Description: checkbook.String("Test memo"),
		Number: checkbook.CheckPreviewParamsNumberUnion{
			OfString: checkbook.String("string"),
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckPrint(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Check.Print(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckTriggerWebhookWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	err := client.Check.TriggerWebhook(
		context.TODO(),
		"check_id",
		checkbook.CheckTriggerWebhookParams{
			Status: checkbook.CheckTriggerWebhookParamsStatusPaid,
			Options: checkbook.CheckTriggerWebhookParamsOptions{
				ReturnCode: "R01",
			},
		},
	)
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestCheckVoid(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	err := client.Check.Void(context.TODO(), "check_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

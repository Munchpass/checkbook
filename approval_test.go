// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

package checkbook_test

import (
	"context"
	"errors"
	"os"
	"testing"
	"time"

	"github.com/Munchpass/checkbook"
	"github.com/Munchpass/checkbook/internal/testutil"
	"github.com/Munchpass/checkbook/option"
)

func TestApprovalGet(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.Get(context.TODO(), "approval_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalUpdateWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	err := client.Approval.Update(
		context.TODO(),
		"approval_id",
		checkbook.ApprovalUpdateParams{
			Account:     checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Amount:      checkbook.Float(0.01),
			Description: checkbook.String("description"),
			Name:        checkbook.String("xx"),
			Number:      checkbook.String("number"),
			Recipient:   checkbook.String("recipient"),
		},
	)
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalListWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.List(context.TODO(), checkbook.ApprovalListParams{
		Direction: checkbook.ApprovalListParamsDirectionOutgoing,
		EndDate:   checkbook.Time(time.Now()),
		Page:      checkbook.Int(1),
		PerPage:   50,
		Q:         checkbook.String("payment"),
		Sort:      checkbook.ApprovalListParamsSortPlusDate,
		StartDate: checkbook.Time(time.Now()),
		Status:    checkbook.ApprovalListParamsStatusPaid,
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalDelete(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	err := client.Approval.Delete(context.TODO(), "approval_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalNewDigitalWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.NewDigital(context.TODO(), checkbook.ApprovalNewDigitalParams{
		CreateDigitalCheck: checkbook.CreateDigitalCheckParam{
			Amount: 150,
			Name:   "Dwight Schrute",
			Recipient: checkbook.CreateDigitalCheckRecipientUnionParam{
				OfString: checkbook.String("dwight@example.com"),
			},
			Account: checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Attachment: checkbook.CreateDigitalCheckAttachmentUnionParam{
				OfAttachment: &checkbook.AttachmentParam{
					Content:  "",
					Filename: "attachment.pdf",
				},
			},
			Comment:        checkbook.String("comment"),
			DepositOptions: []string{"BANK", "RTP"},
			Description:    checkbook.String("Example memo"),
			Number: checkbook.CreateDigitalCheckNumberUnionParam{
				OfString: checkbook.String("5001"),
			},
			Pin: checkbook.PinParam{
				Description: "Please enter your payments pin #",
				Value:       "1234",
			},
			RemittanceAdvice: []checkbook.RemittanceAdviceParam{{
				ID:          "id",
				Date:        "date",
				Amount:      checkbook.Float(0),
				Description: checkbook.String("description"),
			}},
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalNewMultiWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.NewMulti(context.TODO(), checkbook.ApprovalNewMultiParams{
		CreateMultiCheck: checkbook.CreateMultiCheckParam{
			Amount: 5.23,
			Recipients: []checkbook.MultiRecipientParam{{
				Name:        "Widgets Inc.",
				Recipient:   "testing@example.com",
				EndorseOnly: checkbook.Bool(true),
				Pin: checkbook.PinParam{
					Description: "Please enter your payments pin #",
					Value:       "1234",
				},
			}, {
				Name:        "Widgets Inc.",
				Recipient:   "testing@example.com",
				EndorseOnly: checkbook.Bool(true),
				Pin: checkbook.PinParam{
					Description: "Please enter your payments pin #",
					Value:       "1234",
				},
			}},
			Account: checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Attachment: checkbook.CreateMultiCheckAttachmentUnionParam{
				OfAttachment: &checkbook.AttachmentParam{
					Content:  "",
					Filename: "attachment.pdf",
				},
			},
			Comment:        checkbook.String("comment"),
			DepositOptions: []string{"PRINT"},
			Description:    checkbook.String("Example memo"),
			Number: checkbook.CreateMultiCheckNumberUnionParam{
				OfString: checkbook.String("5001"),
			},
			RemittanceAdvice: []checkbook.RemittanceAdviceParam{{
				ID:          "id",
				Date:        "date",
				Amount:      checkbook.Float(0),
				Description: checkbook.String("description"),
			}},
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalNewPhysicalWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.NewPhysical(context.TODO(), checkbook.ApprovalNewPhysicalParams{
		CreatePhysicalCheck: checkbook.CreatePhysicalCheckParam{
			Amount: 5.23,
			Name:   "Widgets Inc.",
			Recipient: checkbook.PhysicalCheckRecipientParam{
				City:    "Anytown",
				Line1:   "1234 N. 1st Street",
				State:   "CA",
				Zip:     "55555",
				Country: checkbook.String("country"),
				Line2:   checkbook.String("Suite 56"),
				Name:    checkbook.String("Widgets Inc."),
			},
			Account: checkbook.String("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"),
			Attachment: checkbook.CreatePhysicalCheckAttachmentUnionParam{
				OfAttachment: &checkbook.AttachmentParam{
					Content:  "",
					Filename: "attachment.pdf",
				},
			},
			Comment:     checkbook.String("comment"),
			Description: checkbook.String("Example memo"),
			MailType:    checkbook.CreatePhysicalCheckMailTypeUspsFirstClass,
			Number: checkbook.CreatePhysicalCheckNumberUnionParam{
				OfString: checkbook.String("5001"),
			},
			RemittanceAdvice: checkbook.CreatePhysicalCheckRemittanceAdviceUnionParam{
				OfString: checkbook.String("string"),
			},
		},
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalRelease(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.Release(context.TODO(), checkbook.ApprovalReleaseParams{
		ID: "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
	})
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}

func TestApprovalGetAttachment(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	if !testutil.CheckTestServer(t, baseURL) {
		return
	}
	client := checkbook.NewClient(
		option.WithBaseURL(baseURL),
		option.WithAPIKey("My API Key"),
	)
	_, err := client.Approval.GetAttachment(context.TODO(), "approval_id")
	if err != nil {
		var apierr *checkbook.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
		}
		t.Fatalf("err should be nil: %s", err.Error())
	}
}
